...
public:
  ...
  void SetUp() override
  {
    ...
    collision_evaluator =
        std::make_shared<trajopt::SingleTimestepCollisionEvaluator>(kin, env, adj_map, Eigen::Isometry3d::Identity(), config);
    ...
    constraint = std::make_shared<trajopt::DiscreteCollisionConstraintIfopt>(collision_evaluator, GradientCombineMethod::SUM, var0);
...
TEST_F(CollisionUnit, GetSetBounds)  // NOLINT
{
  ...
  // Check that setting bounds works
  {
    ...
    auto constraint_2 = std::make_shared<trajopt::DiscreteCollisionConstraintIfopt>(collision_evaluator, GradientCombineMethod::SUM, var0);
